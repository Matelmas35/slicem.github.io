...
var adBreakInterval = 999999999; // Change to 180000 for live
...
function cmgRewardsAdBreak() {
  ...
  sendResizeMessage();
  isRewardAd = false; // Set the flag when loading a rewards ad
}
...
function adBreakSpecificLogic() {
  midRollAdStarted = false;
  midroll_ads_timer = 0; // 8 seconds timer
  cmgAInvD = false;
...
function adRewardsBreakSpecificLogic() {
  midRollAdStarted = false;
  midroll_ads_timer = 0; // 30 seconds timer
  document.dispatchEvent(adBreakStart);
  cmgAInvD = false;
